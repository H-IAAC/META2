...
from scratch.strategic import CEKDLossPlugin, FullyConnectedNetwork, Microtransformer, BaseConvolutionalModel, HARTransformer, CrossAttnHARTransformer, LoraHARTransformer
from scratch.strategic import PlasticityStrategy, MetaPlasticityStrategy, MetaPlasticityLora
...
if __name__ == "__main__":
    ...
    if exp.exp_parser.get("benchmark", "name") == "UCIHAR_TI":      
        ...
        model = LoraHARTransformer((1, 128, 9), 6, 1, 6, dropout=0.15, sensor_group=1, plas=exp.exp_parser.getfloat('training', 'meta_plasticity'))
        num_classes = 6
        '''
    ...
    if exp.exp_parser.get("benchmark", "name") == "PAMAP_TI":
        ...
        model = LoraHARTransformer((1, 104, 27), 8, 1, 12, dropout=0.15, sensor_group=1, plas=exp.exp_parser.getfloat('training', 'meta_plasticity'))
    ...
    if exp.exp_parser.get("benchmark", "name") == "DSADS_TI":
        ...
        model = LoraHARTransformer((1, 125, 45), 2, 1, 19, dropout=0.15, sensor_group=1, plas=exp.exp_parser.getfloat('training', 'meta_plasticity'))
    ...
    if exp.exp_parser.get("benchmark", "name") == "HAPT_TI":
        ...
        #model = LoraHARTransformer((1, 128, 6), 4, 1, 12, dropout=0.15, sensor_group=1, plas=exp.exp_parser.getfloat('training', 'meta_plasticity'))
        model = HARTransformer((1, 128, 6), 4, 1, 12, dropout=0.15, sensor_group=1)
        num_classes=12
    ...
    if exp.exp_parser.getfloat('training', 'meta_plasticity') < 1:
        # print("Choosing metaplas")
        # strategy = MetaPlasticityLora(model, optimizer, loss_plugin,
