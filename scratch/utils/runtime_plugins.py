...
class ClassPrecisionPlugin(SupervisedPlugin):
    ...
    def after_eval_forward(self, strategy: "SupervisedTemplate", **kwargs):
      ...
      with torch.no_grad():
        ...
        self.predictions = torch.cat((self.predictions, torch.argmax(strategy.mb_output, dim = 1)))
    ...
    def after_eval(self, strategy: "SupervisedTemplate", **kwargs):
      ...
      self.classifications.append(classification_report(self.true, self.predictions))
      self.confusion_matrices.append(confusion_matrix(self.true, self.predictions))
      self.predictions = torch.empty((0), dtype = torch.int32)
      self.true = torch.empty((0), dtype = torch.int32)
